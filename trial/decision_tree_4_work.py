...
def growDecisionTreeFrom(rows, evaluationFunction=entropy, curr_depth=0):
    ...
    if bestGain > 0:
        trueBranch = growDecisionTreeFrom(bestSets[0], evaluationFunction, curr_depth + 1)
        falseBranch = growDecisionTreeFrom(bestSets[1], evaluationFunction, curr_depth + 1)
...
def classify(observations, tree, dataMissing=False):
    ...
    def classifyWithoutMissingData(observations, tree):
        if tree.results != None:  # leaf
            predict_value = max(tree.results, key=lambda k: tree.results[k])
...
def dotgraph(decisionTree):
    ...
    # print(decisionTree.col)
    # print(decisionTree.value)
    # print(decisionTree.trueBranch)
    # print(decisionTree.falseBranch)
    # print(decisionTree.results)
    # print(decisionTree.summary)
...
if __name__ == '__main__':
    ...
    if example == 1:
        ...
        bHeader = True
        # the bigger example
        dcHeadings, trainingData = loadCSV('../data/train.csv') # demo data from matlab
        ...
        # print(classify([1,0,1,0,0,132,0,1,9,213,254], decisionTree)) # dataMissing=False is the default setting
        ...
        for x in X:
            # print(list(x))
            y_predict.append(classify(x, decisionTree))
